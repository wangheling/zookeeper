...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public enum LearnerType {
        //leader和follower都是参与者，observer是观察者
    ...
    public synchronized void start() {
        //加载数据，将myid，zxid、epoch从磁盘加载到内存
        loadDataBase();
        //cnxnFacotory?  跟通信有关系. ->暴露一个2181的端口号
        cnxnFactory.start();
        //开始leader选举-> 启动一个投票的监听、初始化一个选举算法FastLeader（选票发送队列，接收队列和消息发送manager）
        startLeaderElection();
    ...
    synchronized public void startLeaderElection() {
        try {
            //构建一个选举票据， （myid ,zxid ,epoch）,用来投票的。
            //id就是上一步loadDataBase()
        ...
        for (QuorumServer p : getView().values()) {
            if (p.id == myid) {
                //如果和自己myid一致。第一次投票都会先投自己
                myQuorumAddr = p.addr;
                break;
            }
        }
        if (myQuorumAddr == null) {
            //zoo.cfg里没有配置当前机器ip，异常
            throw new RuntimeException("My id " + myid + " not in the peer list");
        }
        //选举的策略，默认是3，可以在zoo.cfg配置
        if (electionType == 0) {
            try {
                    ...
                    case LOOKING:
                        ...
                        if (Boolean.getBoolean("readonlymode.enabled")) {
                            ...
                            try {
                                ...
                                //makeLEStrategy()前面选择的算法，默认3 FastLeaderElection
                                //lookForLeader()开启新一轮的选举，当zk server的state是looking的时候调用
                                setCurrentVote(makeLEStrategy().lookForLeader());
                            } catch (Exception e) {
                                LOG.warn("Unexpected exception", e);
                                setPeerState(ServerState.LOOKING);
